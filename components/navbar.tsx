  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? "bg-background/85 backdrop-blur-2xl border-b border-primary/10" : "bg-transparent"
      }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        ...
        <div className="hidden items-center gap-8 md:flex">
          ...
          <a href="/CV_Karim_Daaboul__2_.pdf" download
            className="rounded border border-primary/40 px-5 py-2 text-xs font-mono tracking-widest text-primary uppercase transition-all hover:border-primary hover:bg-primary/10">
            Resume
          </a>
        </div>
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="flex flex-col gap-6 px-6 py-8">
              {navLinks.map((link, i) => (
                <button key={link.id} onClick={() => scrollTo(link.id)}
                  className="text-left font-mono text-xs tracking-widest text-muted-foreground uppercase hover:text-primary transition-colors">
